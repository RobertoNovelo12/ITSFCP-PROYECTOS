...
class fuenteControlador
{
    ...
    /**
     * Obtiene listado de Fuente con filtro opcional.
     */
    ...
    /**
     * Obtiene datos de un Fuente para edición.
     */
    public function indexEditar($rol, $id_fuente): array
    {
        global $conn;
        try {
            if (!$this->esSupervisor($rol)) return [];
            $id = filter_var($id_fuente, FILTER_VALIDATE_INT);
            if (!$id) return [];
            $obj = new Fuente($conn);
    ...
    /**
     * Obtiene detalles de un Fuente específico.
     */
    public function indexDetalles($rol, $id_fuente): array
    {
        global $conn;
        try {
            if (!$this->esSupervisor($rol)) return [];
    ...
    /**
     * Desactiva un Fuente (borrado lógico).
     */
    public function eliminar($rol, $id_fuente)
    {
        if (!$this->esSupervisor($rol)) {
            throw new Exception("No tienes permiso para eliminar Fuente.");
        }
        if (!$id_fuente) {
        ...
        try {
            $obj = new Fuente($conn);
            $obj->obtenerPorId((int)$id_fuente);
            $filas = $obj->eliminar_fuente((int)$id_fuente);
    ...
    private function obtenerbotones($tipo, $id1 = null)
    {
        ...
        switch ($tipo) {
            case 'Editar Fuente':
                $boton = '<a href="editar.php?id_fuente=' . $id1 . '" type="button" class="btn btn-warning" data-bs-toggle="tooltip" data-bs-placement="top"
        data-bs-custom-class="custom-tooltip" data-bs-title="Editar Fuente"><svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="currentColor" class="bi bi-pencil-square" viewBox="0 0 16 16">
  <path d="M15.502 1.94a.5.5 0 0 1 0 .706L14.459 3.69l-2-2L13.502.646a.5.5 0 0 1 .707 0l1.293 1.293zm-1.75 2.456-2-2L4.939 9.21a.5.5 0 0 0-.121.196l-.805 2.414a.25.25 0 0 0 .316.316l2.414-.805a.5.5 0 0 0 .196-.12l6.813-6.814z"/>
  <path fill-rule="evenodd" d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5z"/>
</svg></a>';
                break;
            case 'Detalles':
                $boton = '<a href="detalles.php?id_fuente=' . $id1 . '" type="button" class="btn btn-info" data-bs-toggle="tooltip" data-bs-placement="top"
        data-bs-custom-class="custom-tooltip" data-bs-title="Ver detalles de Fuente"><svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="currentColor" class="bi bi-eye-fill" style="padding:0px;margin:auto;" viewBox="0 0 16 16">
  <path d="M10.5 8a2.5 2.5 0 1 1-5 0 2.5 2.5 0 0 1 5 0"/><path d="M0 8s3-5.5 8-5.5S16 8 16 8s-3 5.5-8 5.5S0 8 0 8m8 3.5a3.5 3.5 0 1 0 0-7 3.5 3.5 0 0 0 0 7"/></svg></a>';
                break;
            case 'Desactivar':
                $boton = '<a href="tabla.php?&id_fuente=' . $id1 . '&action=desactivar_fuente" type="button" class="btn btn-danger" data-bs-toggle="tooltip" data-bs-placement="top"
        data-bs-custom-class="custom-tooltip" data-bs-title="Desactivar Fuente"><svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="currentColor" class="bi bi-x-circle-fill" viewBox="0 0 16 16">
  <path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0M5.354 4.646a.5.5 0 1 0-.708.708L7.293 8l-2.647 2.646a.5.5 0 0 0 .708.708L8 8.707l2.646 2.647a.5.5 0 0 0 .708-.708L8.707 8l2.647-2.646a.5.5 0 0 0-.708-.708L8 7.293z"/>
    ...
    public function botonesAccionPrincipal($id, $rol, $estado = null)
    {
        ...
        if (in_array($estado, ["Activo"])) {
            $boton .= $this->obtenerbotones("Editar Fuente", $id);
     ...
     */
    function registrarFuente($rol, $fuente)
    {
        if (!$this->esSupervisor($rol)) return "";
        global $conn;
        ...
        try {
            $obj = new Fuente($conn);
            $obj->bloquear_tabla();
            $verificacion = $obj->verificarFuente($fuente);

            if ($verificacion['activo'] > 0 && $verificacion['desactivado'] > 0) {
                throw new Exception("Registro duplicado");
            }

            $id = $obj->registrarFuente($fuente);
     ...
     */
    function editarFuente($rol, $id_fuente, $fuente)
    {
        if (!$this->esSupervisor($rol)) return "";
        global $conn;
        ...
        try {
            $obj = new Fuente($conn);
            $verificacion = $this->obtenerPorIdDiferente((int)$id_fuente, $fuente);

            if ($verificacion['activo'] > 0 || $verificacion['desactivado'] > 0) {
                throw new Exception("Registro duplicado");
            }

            $id_fuente = $obj->editarFuente($fuente, $id_fuente);
     ...
     * Reactiva un Fuente.
     */
    public function reactivar($rol, $id_fuente)
    {
        if (!$this->esSupervisor($rol)) return "";
        global $conn;
        ...
        try {
            $obj = new Fuente($conn);
            $obj->bloquear_tabla();
            $obj->obtenerPorId($id_fuente, true);
            $obj->reactivar($id_fuente);
    ...
    /**
     * Verifica existencia de conflictos de Fuente.
     */
    public function verificarFuente($fuente): array
    {
        global $conn;
        try {
            if (empty($fuente)) return ["activo" => 0, "desactivado" => 0];
            $obj = new Fuente($conn);
            return $obj->verificarFuente($fuente);
        } catch (Throwable $e) {
            error_log("Error en verificarFuente(): " . $e->getMessage());
            return ["activo" => 0, "desactivado" => 0];
        }
    }

    public function obtenerPorIdDiferente($id_fuente, $fuente): array
    {
        global $conn;
        try {
            if (empty($fuente)) return ["activo" => 0, "desactivado" => 0];
            $obj = new Fuente($conn);
            return $obj->obtenerPorIdDiferente($id_fuente, $fuente);
        } catch (Throwable $e) {
