...
class MisSolicitudes
{
    ...
    // -------------------------------------------------------------------------
    //  CATÁLOGO: periodos con solicitudes del estudiante (para el filtro)
    // -------------------------------------------------------------------------
    ...
    // -------------------------------------------------------------------------
    //  CONTAR solicitudes (para paginación)
    // -------------------------------------------------------------------------
    public function contarSolicitudes(int $id_estudiante, array $filtros): int
    {
        ...
        $sql = "
            SELECT COUNT(DISTINCT sp.id_solicitud_proyecto)
            FROM solicitud_proyecto sp
            JOIN proyectos p ON p.id_proyectos = sp.id_proyectos
    ...
    // -------------------------------------------------------------------------
    //  HILO de comentarios de una solicitud
    // -------------------------------------------------------------------------
    public function obtenerHilo(int $id_solicitud, int $id_estudiante): array
    {
        ...
        ");
        if (!$stmtC) return [];
        $stmtC->bind_param("ii", $id_solicitud, $id_estudiante);
        $stmtC->execute();
        if ($stmtC->get_result()->num_rows === 0) return [];
        $stmtC->close();
    ...
    // -------------------------------------------------------------------------
    //  Datos mínimos para validar antes de guardar respuesta
    // -------------------------------------------------------------------------
    ...
    // -------------------------------------------------------------------------
    //  GUARDAR respuesta del estudiante a correcciones
    // -------------------------------------------------------------------------
    public function guardarRespuesta(
        ...
        try {
            ...
            if ($archivo !== null) {
                ...
                ";
                $stmtD = $this->conn->prepare($sqlDoc);
                if (!$stmtD) throw new Exception($this->conn->error);
                $stmtD->bind_param(
                    "sssssiii",
                ...
                $stmtD->execute();
                $id_documento = $this->conn->insert_id;
                $stmtD->close();
            }
            ...
            $sqlCom = "
                ...
                VALUES (?, ?, 'estudiante', ?, ?)
            ";
            $stmtCom = $this->conn->prepare($sqlCom);
            if (!$stmtCom) throw new Exception($this->conn->error);
            $stmtCom->bind_param("iisi", $id_solicitud, $id_estudiante, $comentario, $id_documento);
            $stmtCom->execute();
            $stmtCom->close();
            ...
            if (!$stmtUpd) throw new Exception($this->conn->error);
            $stmtUpd->bind_param("ii", $id_solicitud, $id_estudiante);
            $stmtUpd->execute();
            $stmtUpd->close();
            ...
            $this->conn->rollback();
            error_log("guardarRespuesta(): " . $e->getMessage());
            return false;
        }
    }
