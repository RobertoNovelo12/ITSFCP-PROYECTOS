...
class ajustesdocumentos
{
    ...
    public function obtenerTablaFiltro($buscar, $filtro): array
    {
         ...
         */
        $sql .= " ORDER BY id_tipo_documento ASC LIMIT ?, ?";
    ...
    public function obtenerCantidad($buscar = null, $filtro = 2): int
    {
        ...
        $sql = "SELECT COUNT(*) AS total FROM tipo_documento";
        $sql .= $this->construirWhere($params, $types, $buscar, $filtro);

        $stmt = $this->con->prepare($sql);

        if (!$stmt) {
            throw new Exception("Error en prepare (obtenerCantidad): " . $this->con->error);
        }
        ...
        if (!$stmt->execute()) {
            throw new Exception("Error en execute (obtenerCantidad): " . $stmt->error);
        }
    ...
    public function obtenerEditar($id_tipo_documento): array
    {
        ...
        $plantilla = $ajuste->obtener_plantilla($id_tipo_documento);



        if (!$plantilla) {
            throw new Exception("El tipo de documento no encontrada");
        }

        $id_plantilla = $plantilla['id_plantilla'];
        //Obtener datos de la configuracion

        $configuracion = $ajuste->obtener_plantillas_documento($id_plantilla);

        if (!$configuracion) {
            throw new Exception("La configuración no encontrada");
        }

        //Obtener datos de la configuracion

        $fuentes = $ajuste->obtener_fuente($id_plantilla);
    ...
    private function obtener_plantilla($id_tipo_documento)
    {
        $sql_plantilla = "SELECT 
                    ...
                    CASE 
                        ...
                        ELSE 'Desconocido'
                    END AS estado
                FROM plantillas_documento AS d
                WHERE d.id_tipo_documento = ?";

        $stmt_p = $this->con->prepare($sql_plantilla);

        if (!$stmt_p) {
            throw new Exception("Error en prepare (obtener_plantilla): " . $this->con->error);
        }

        $stmt_p->bind_param("i", $id_tipo_documento);

        if (!$stmt_p->execute()) {
            throw new Exception("Error en execute (obtener_plantilla): " . $stmt_p->error);
        }
    ...
    private function obtener_plantillas_documento($id_plantilla)
    {
        $sql_c = "SELECT 
                    id_configuracion_documento,
                    nombre,
                    id_fuente,
                    tamaño_hoja,
                    margen_superior,
                    ...
                    tamaño_fuente,
                    interlineado
                FROM configuracion_documento
                WHERE id_plantilla = ?";

        $stmt_C = $this->con->prepare($sql_c);

        if (!$stmt_C) {
            throw new Exception("Error en prepare (obtener_plantillas_documento): " . $this->con->error);
        }

        $stmt_C->bind_param("i", $id_plantilla);

        if (!$stmt_C->execute()) {
            throw new Exception("Error en execute (obtener_plantillas_documento): " . $stmt_C->error);
        }

        $configuracion = $stmt_C->get_result()->fetch_assoc();
    ...
    private function obtener_fuente()
    {
        $sql_f = "SELECT 
                    id_fuente,
                    fuente
        ...
        $stmt_F = $this->con->prepare($sql_f);

        if (!$stmt_F) {
            throw new Exception("Error en prepare (obtener_fuente): " . $this->con->error);
        }

        if (!$stmt_F->execute()) {
            throw new Exception("Error en execute (obtener_fuente): " . $stmt_F->error);
        }

        $fuentes = $stmt_F->get_result()->fetch_assoc();
    ...
    private function obtener_encabezado_y_pie($id_plantilla)
    { //False encabezado 
     ...
     * @throws Exception
     */
    public function verificar(string $nombre): array
    {
