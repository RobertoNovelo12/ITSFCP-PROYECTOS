...
class Proyectos
{
    ...
    private function obtenerProyectosTablaEstudiante(int $id, ?int $filtro, ?string $buscar): string
    {
        ...
        $total         = $this->contarEstudiante($id, $filtro, $buscar);
        ...
        $sql    = '
            SELECT
                ...
                pu.estado AS estado_estudiante, COALESCE(tr.total, 0) AS total
            FROM proyectos proy
    ...
    {
        $sql    = "SELECT COUNT(*) AS total FROM proyectos proy
                   JOIN proyectos_usuarios pu
                       ON pu.id_proyectos = proy.id_proyectos AND pu.id_usuarios = ? AND pu.estado = 'activo'
                   WHERE 1 = 1";
        $params = [$id];
        $types  = 'i';
