<?php
require_once __DIR__ . '/../Model/solicitud_sni_model.php';
...
class SolicitudSniControlador
{
    ...
    {
        global $conn;
        $modelo = new SolicitudSni($conn);
        return [
            'investigador'  => $modelo->obtenerDatosInvestigador($id_usuario),
            'niveles_sni'   => $modelo->obtenerNivelesSni(),
            'pendiente_sni' => $modelo->tieneSolicitudPendiente($id_usuario),
        ];
    }
    ...
    public function crearSolicitud($id_usuario)
    {
        if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
            header("Location: configuracion_sni.php");
            exit;
        }
        ...
        $valor_actual = (int)$inv['id_nivel_sni'];
        $archivo      = $_FILES['documento'] ?? [];

        $resultado = $modelo->crearSolicitud($id_usuario, $valor_actual, $valor_nuevo, $archivo);

        if ($resultado['ok']) {
            header("Location: configuracion_sni.php?msg=enviado");
        } else {
            header("Location: configuracion_sni.php?error=" . urlencode($resultado['msg']));
        }
        exit;
    }
    ...
    {
        $html  = '<a href="detalles_sni.php?id_solicitud=' . $id_solicitud . '"
        ...
        if ($estado === 'pendiente') {
            ...
            $html .= '<a href="respuesta_sni.php?id_solicitud=' . $id_solicitud . '"
    ...
    private function enviarCorreo($destinatario, $nombre, $estado, $comentario)
    {
        ...
        try {
            ...
            $mail->isHTML(true);
            $mail->Subject = 'Resultado de tu solicitud de Nivel SNI';
            ...
            <body style='font-family:Arial,sans-serif;background:#f8f9fa;padding:20px;'>
              <div style='max-width:600px;margin:auto;background:#fff;border-radius:8px;padding:30px;box-shadow:0 2px 8px rgba(0,0,0,.1);'>
                ...
                <p>Tu solicitud de cambio de <strong>Nivel SNI</strong> ha sido revisada.</p>
            ...
            $mail->AltBody = "Hola $nombre. Tu solicitud de Nivel SNI fue: $estadoTexto. $comentario";
            $mail->send();
        } catch (Exception $e) {
            error_log("Error enviando correo SNI: " . $mail->ErrorInfo);
        }
    }
}
