...
// Subáreas
$subareas = $conn->query("SELECT id_subarea, nombre_subarea, id_area FROM subareas_conocimiento ORDER BY nombre_subarea");
...
<?php if ($rol === 'estudiante'): ?>
    <form class="form-perfil" action="/Modules/Usuarios/Controller/procesar_solicitud.php" method="POST">
...
<?php elseif ($rol === 'investigador'): ?>
    <form class="form-perfil" action="/Modules/Usuarios/Controller/procesar_solicitud.php" method="POST">
        ...
        <div class="input-group">
            ...
            <select name="area" id="area" onchange="filtrarSubareas()" required>
                <option value="" disabled selected>Seleccione área...</option>
                <?php if ($areas): $areas->data_seek(0); while ($a = $areas->fetch_assoc()): ?>
                    <option value="<?= $a['id_area'] ?>"><?= $a['nombre_area'] ?></option>
                <?php endwhile; endif; ?>
            </select>
        </div>
        ...
        <div class="input-group">
            ...
            <select name="subarea" id="subarea" required>
                ...
                <?php if ($subareas): $subareas->data_seek(0); while ($s = $subareas->fetch_assoc()): ?>
                    <option value="<?= $s['id_subarea'] ?>" data-area="<?= $s['id_area'] ?>"><?= $s['nombre_subarea'] ?></option>
                <?php endwhile; endif; ?>
            </select>
        </div>
        ...
        <div class="input-group">
            ...
            <select name="nivel_sni" required>
                ...
                <?php if ($nivelesSNI): $nivelesSNI->data_seek(0); while ($n = $nivelesSNI->fetch_assoc()): ?>
        ...
        <div class="input-group">
            ...
            <select name="grado" required>
                ...
                <?php if ($grados): $grados->data_seek(0); while ($g = $grados->fetch_assoc()): ?>
        ...
        <div class="input-group">
            ...
            <select name="linea" required>
                ...
                <?php if ($lineas): $lineas->data_seek(0); while ($l = $lineas->fetch_assoc()): ?>
...
<?php elseif ($rol === 'supervisor'): ?>
    <form class="form-perfil" action="/Modules/Usuarios/Controller/procesar_solicitud.php" method="POST" enctype="multipart/form-data">
        <input type="hidden" name="rol" value="supervisor">

        <div class="input-group">
            <label class="label-select">Descargar formato de solicitud</label><br>
            <a href="/public/docs/formato_solicitud.pdf" class="submit-btn" download>Descargar Formato</a>
        </div>
...
<?php else: ?>
    <p>No se reconoce el tipo de usuario. <a href="/Modules/Usuarios/Views/crear_perfil.php">Volver</a></p>
<?php endif; ?>
