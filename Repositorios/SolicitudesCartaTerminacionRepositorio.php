<?php
// Repositorios/SolicitudesCartaTerminacionRepositorio.php

require_once __DIR__ . '/../Modelos/BaseModelo.php';

/**
 * SolicitudesCartaTerminacionRepositorio
 *
 * Responsabilidad exclusiva: ejecutar consultas SQL del módulo de cartas
 * de terminación de estudiantes.
 * No contiene lógica de negocio.
 *
 * aprobarCarta() y rechazarCarta() gestionan transacciones propias porque son
 * operaciones de persistencia compuestas e indivisibles. El modelo las delega
 * completamente y solo recibe el array de resultado.
 *
...
class SolicitudesCartaTerminacionRepositorio extends BaseModelo
{
    ...
    /**
     * El $where_periodo llega ya construido desde el modelo
     * (cadena vacía o "AND p.id_periodos = ?" con su param/type).
     */
    public function resumenCartas(string $where_periodo, array $params, string $types): array
    {
        $fila = $this->ejecutar(
            "SELECT
                COUNT(*) AS total,
        ...
        return $fila ?: ['total' => 0, 'pendientes' => 0, 'aprobadas' => 0, 'rechazadas' => 0];
    }
    ...
    public function listarCartas(string $base_where, array $params, string $types, int $desde, int $por_pagina): array
    {
        $params[] = $desde;
        $params[] = $por_pagina;
        $types   .= 'ii';
