<?php
require_once __DIR__ . '/../../Controladores/solicitudSniControlador.php';

session_start();
$id_usuario = $_SESSION['id_usuario'] ?? 0;
if (!$id_usuario) { header("Location: login.php"); exit; }

$ctrl = new SolicitudSniControlador();

if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['accion'] ?? '') === 'crear_sni') {
...
$niveles   = $datos['niveles_sni'];
$pendiente = $datos['pendiente_sni'];
...
<div class="container-fluid py-4" style="max-width:95%;">
    ...
    <div class="d-flex flex-wrap justify-content-between align-items-center mb-4 gap-2">
        <div>
            <h4 class="mb-0 fw-semibold">Configuración Nivel SNI</h4>
            <small class="text-muted">Solicita cambios y revisa tu historial</small>
        </div>
    </div>
    ...
    <div class="card border-0 shadow-sm mb-4">
        ...
        <div class="card-body">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    <small class="text-muted">Nivel SNI actual</small>
                    <div>
                        <?php if (!empty($inv['nivel_sni_nombre'])): ?>
                            <span class="badge text-bg-primary">
                                <?= htmlspecialchars($inv['nivel_sni_nombre']) ?>
                            </span>
                        <?php else: ?>
    ...
    <!-- FORMULARIO -->
    <div class="card border-0 shadow-sm mb-4">
        <div class="card-header"><b>Solicitar cambio de nivel SNI</b></div>
        <div class="card-body">
            ...
            <?php else: ?>
                ...
                <form method="POST" enctype="multipart/form-data">
                    <input type="hidden" name="accion" value="crear_sni">

                    <div class="row g-3">

                        <div class="col-md-6">
                            <label class="form-label fw-semibold">Nuevo nivel SNI</label>
                            <select class="form-select" name="valor_nuevo_id" required>
                                <option value="">Selecciona</option>
                                <?php foreach ($niveles as $n): ?>
                                    <option value="<?= $n['id_nivel'] ?>"
                                        <?= (int)$n['id_nivel'] === (int)$inv['id_nivel_sni'] ? 'disabled' : '' ?>>
                ...
                <?php if ($pagina['total_paginas'] > 1): ?>
                    <nav class="mt-4">
                        <ul class="pagination pagination-sm justify-content-center flex-wrap">
                            ...
                            <li class="page-item <?= $pagina['pagina'] <= 1 ? 'disabled' : '' ?>">
                                <a class="page-link" href="?pagina=<?= $pagina['pagina'] - 1 ?>">
                                    ‹
                                </a>
                            </li>

                            <?php for ($p = 1; $p <= $pagina['total_paginas']; $p++): ?>
                                <li class="page-item <?= $p === $pagina['pagina'] ? 'active' : '' ?>">
                                    <a class="page-link" href="?pagina=<?= $p ?>"><?= $p ?></a>
                                </li>
                            <?php endfor; ?>

                            <li class="page-item <?= $pagina['pagina'] >= $pagina['total_paginas'] ? 'disabled' : '' ?>">
                                <a class="page-link" href="?pagina=<?= $pagina['pagina'] + 1 ?>">
                                    ›
                                </a>
                            </li>
                        ...
                        <p class="text-center text-muted small mb-0">
                            Página <?= $pagina['pagina'] ?> de <?= $pagina['total_paginas'] ?>
                        </p>
                    </nav>
                <?php endif; ?>
