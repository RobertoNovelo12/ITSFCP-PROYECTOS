...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST) && $action === 'Registrar') {
    $fuente = $_POST['Fuente'];
    $estadoVista = $fuenteControlador->verificarFuente($fuente);

    if ($estadoVista['activo'] == 0 && $estadoVista['desactivado'] == 0) {
        $fuenteControlador->registrarFuente($rol, $fuente);
    } else {
        $mensaje = "Ya hay un Fuente con ese nombre, intente con otro";
    }
}
