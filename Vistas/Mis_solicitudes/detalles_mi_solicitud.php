...
<div class="container-fluid py-4 ancho_container">
    ...
    <?php if ($msg_ok): ?>
        <div class="alert alert-success alert-dismissible fade show">
            <i class="<?= $iconos['detalles']['exito'] ?> me-2"></i><?= $msg_ok ?>
            <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
        </div>
    <?php endif; ?>
    <?php if ($msg_err): ?>
        <div class="alert alert-danger alert-dismissible fade show">
            <i class="<?= $iconos['detalles']['advertencia'] ?> me-2"></i><?= $msg_err ?>
            <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
        </div>
    <?php endif; ?>
    ...
    <div class="card shadow-sm mb-4 border-primary">
        <div class="card-header text-white">
            <h5 class="mb-0">
                <i class="bi bi-diagram-3 me-2"></i>Seguimiento del estudiante
            </h5>
        </div>
                ...
                <div class="col-md-4 p-3">
                    ...
                    <?php if ($fase2_ok && $id_seg_cierre && in_array($e3_estado, ['pendiente', 'proceso', 'correcciones'])): ?>
                        <div class="d-flex gap-1 flex-wrap mt-2">
                            <a href="accion_cierre.php?id_seg=<?= $id_seg_cierre ?>&id_sol=<?= $id_solicitud ?>&estado=completado"
                                ...
                                <i class="<?= $iconos['tabla']['aprobar'] ?>"></i> Aprobar
                            </a>
                    ...
                    <?php elseif (!$fase2_ok): ?>
                        <div class="text-muted small">
                            <i class="<?= $iconos['detalles']['bloqueado'] ?>"></i> Disponible al completar el desarrollo.
                        </div>
                    <?php elseif ($e3_estado === 'completado'): ?>
                        <div class="alert alert-success py-1 px-2 mt-2 small mb-0">
                            <i class="<?= $iconos['detalles']['exito'] ?>"></i> Cierre aprobado. Proyecto finalizado.
                        </div>
                    <?php endif; ?>
                </div>
    ...
    <?php if (!empty($documentos)): ?>
        <div class="card shadow-sm mb-4">
            <div class="card-header text-white">
                <h5 class="mb-0">
                    <i class="<?= $iconos['tabla']['adjunto'] ?> me-2"></i>Documentos subidos por el estudiante
                </h5>
            </div>
            <div class="card-body">
                <div class="d-flex flex-wrap gap-2">
                    <?php foreach ($documentos as $doc): ?>
                        <a href="/<?= htmlspecialchars($doc['ruta']) ?>"
                            ...
                            <i class="<?= $iconos['tabla']['descargar'] ?> me-1"></i>
    ...
    <?php if (!empty($sol['carta_ruta'])): ?>
        <div class="card shadow-sm mb-4">
            <div class="card-header text-white">
                <h5 class="mb-0">
                    <i class="<?= $iconos['detalles']['exito_verificado'] ?> me-2"></i>Carta compromiso
                </h5>
            </div>
            <div class="card-body">
                <a href="/<?= htmlspecialchars($sol['carta_ruta']) ?>"
                    target="_blank" class="btn btn-outline-primary">
                    <i class="<?= $iconos['tabla']['bajar'] ?> me-1"></i>
    ...
    <div class="card shadow-sm mb-4">
        <div class="card-header text-white">
            <h5 class="mb-0">
                <i class="<?= $iconos['detalles']['informacion'] ?> me-2"></i>Información del estudiante
            </h5>
        </div>
        <div class="card-body">
            <div class="row">
    ...
    <div class="card shadow-sm mb-4">
        <div class="card-header text-white">
            <h5 class="mb-0">
                <i class="<?= $iconos['detalles']['proyecto'] ?> me-2"></i>Proyecto solicitado
            </h5>
        </div>
        <div class="card-body">
            <div class="row">
    ...
    <div class="row g-3 mb-4">
        <div class="col-md-6">
            <div class="card shadow-sm h-100">
                <div class="card-header text-white">
                    <h5 class="mb-0">
                        <i class="<?= $iconos['detalles']['informacion'] ?> me-2"></i>Motivación
                    </h5>
                </div>
                <div class="card-body">
        ...
        <div class="col-md-6">
            <div class="card shadow-sm h-100">
                <div class="card-header text-white">
                    <h5 class="mb-0">
                        <i class="<?= $iconos['detalles']['subinformacion'] ?> me-2"></i>Experiencia
                    </h5>
                </div>
                <div class="card-body">
    ...
    <div class="card shadow-sm mb-4">
        <div class="card-header text-white">
            <h5 class="mb-0">
                <i class="<?= $iconos['tabla']['comentarios'] ?> me-2"></i>Historial de comentarios
            </h5>
        </div>
        <div class="card-body">
                    ...
                    <div class="border rounded p-3 mb-2 <?= $c['tipo'] === 'investigador' ? 'bg-light' : 'bg-white' ?>">
                        ...
                        <?php if (!empty($c['archivo_nombre'])): ?>
                            <a href="/<?= htmlspecialchars($c['archivo_ruta']) ?>"
                                ...
                                <i class="<?= $iconos['tabla']['adjunto'] ?>"></i>
            ...
            <?php else: ?>
                <p class="text-muted">
                    <i class="<?= $iconos['tabla']['comentarios'] ?> me-1"></i>Sin comentarios aún.
                </p>
            <?php endif; ?>
        </div>
    </div>
    ...
    <?php if (in_array($sol['estado'], ['pendiente', 'en_revision', 'correcciones'])): ?>
        <div class="card shadow-sm mb-4">
            <div class="card-header">
                <h5 class="mb-0">
                    <i class="<?= $iconos['detalles']['informacion'] ?> me-2"></i>Acciones sobre la solicitud
                </h5>
            </div>
            <div class="card-body d-flex gap-2 flex-wrap">
                ...
                <a href="accion_solicitud.php?id=<?= $id_solicitud ?>&tipo=correcciones"
                    ...
                    <i class="<?= $iconos['tabla']['editar'] ?>"></i> Pedir correcciones
                </a>
                <a href="accion_solicitud.php?id=<?= $id_solicitud ?>&tipo=rechazar"
