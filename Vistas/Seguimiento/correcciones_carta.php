...
<div class="hilo-wrap">
    ...
    <div class="d-flex align-items-center mb-4 gap-3 flex-wrap">
        ...
        <h4 class="mb-0">Carta de terminación #<?= $id_cierre_est ?></h4>
...
<style>
.hilo-wrap { max-width: 820px; margin: 0 auto; }
...
.est-proceso     { background: var(--badge-poraprobar-bg); color: var(--badge-poraprobar-color); border: 1px solid var(--badge-poraprobar-border); }
.est-aceptado    { background: var(--badge-activo-bg);     color: var(--badge-activo-color);     border: 1px solid var(--badge-activo-border); }
.est-correcciones{ background: var(--badge-porcerrar-bg);  color: var(--badge-porcerrar-color);  border: 1px solid var(--badge-porcerrar-border); }
...
document.getElementById('btnEnviarCorrecciones')?.addEventListener('click', function () {
    ...
    .then(data => {
        ...
        if (data.ok) {
            ...
            const now  = new Date();
            const hora = now.toLocaleDateString('es-MX') + ' ' + now.toLocaleTimeString('es-MX', {hour:'2-digit',minute:'2-digit'});
