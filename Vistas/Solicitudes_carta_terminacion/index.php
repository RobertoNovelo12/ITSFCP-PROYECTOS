<?php
// Vistas/cartas_terminacion/index.php
// Tabla principal de solicitudes de carta de terminación (solo supervisor)
...
if (isset($_GET['mensaje'])) {
    if ($_GET['mensaje'] === 'aprobado') {
        $msg_clase = 'success';
        $msg_texto = 'Carta de terminación aprobada correctamente. El estudiante ha concluido su participación.';
    } elseif ($_GET['mensaje'] === 'rechazado') {
        $msg_clase = 'warning';
        $msg_texto = 'Carta de terminación rechazada. El estudiante fue notificado para reenviarla.';
    }
}
if (isset($_GET['error']) && $_GET['error']) {
    $msg_clase = 'danger';
    $msg_texto = htmlspecialchars($_GET['error']);
}
?>
...
<div class="container-fluid py-4" style="max-width:95%;">
    ...
    <!-- MENSAJES DE ALERTA -->
    <?php if ($msg_texto): ?>
        <div class="alert alert-<?= $msg_clase ?> alert-dismissible fade show" role="alert">
            <?= $msg_texto ?>
                    ...
                    <?php foreach ($solicitudes as $sol): ?>
                        <div class="card mb-3 shadow-sm">
                            <div class="card-body">
                                ...
                                <p class="mb-1 small">
                                    <strong>Estudiante:</strong> <?= htmlspecialchars($sol['estudiante']) ?>
                                </p>
                                <p class="mb-1 small">
                                    <strong>Investigador:</strong> <?= htmlspecialchars($sol['investigador']) ?>
                                </p>
                                <p class="mb-1 small">
                                    <strong>Periodo:</strong> <?= htmlspecialchars($sol['periodo']) ?>
                                </p>
                                <p class="mb-1 small">
                                    <strong>Solicitud:</strong> <?= htmlspecialchars($sol['fecha_solicitud']) ?>
                                </p>
                                <p class="mb-2 small">
...
<?php
// Acción GET: aprobar desde botón de tabla
if (isset($_GET['action']) && $_GET['action'] === 'aprobar' && isset($_GET['id'])) {
    // Se procesa antes del ob_start pero se redirige; incluimos la lógica aquí para
    // mantener el flujo en un solo archivo. En producción puede separarse.
    $ctrl->aprobarCarta(intval($_GET['id']), $id_usuario, $rol);
}
